...
- `scripts/build-app.sh` stamps ordinary local bundles with a `-local.<commit>` display suffix and the HEAD commit timestamp as the build number, matching the Edge build of that commit, so later Edge builds are still offered to a Mac that ran a local build. Uncommitted changes use the current time instead so the feed cannot replace them with the older published artifact. Distribution workflows set `CADENCE_DISTRIBUTION_BUILD=1` only after configuring their immutable version.
...
`CADENCE_SMOKE_TEST=notes` exercises the meeting note taker: it starts notes, speaks both sides of a short exchange through the speakers, stops, prints the saved lines, and exits 0 when the transcript contains both phrases. The first run on a Mac shows the one-time System Audio Recording prompt, which someone must answer; add `CADENCE_NOTES_MICROPHONE_ONLY=1` to skip the system tap and verify the rest without that grant.
