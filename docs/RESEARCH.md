...
After the queue drains, Cadence compares the exact accessible document mutation and cursor when both are available. It no longer collapses whitespace during this proof, because an editor that substituted spaces for a requested line break had not delivered the same text. When only a cursor is available, its exact advancement remains useful evidence. Quartz has no delivery receipt, so fully opaque editors remain “unverifiable” rather than being reported as successful. A definite mismatch, unchanged editor, or posting failure produces the recovery card.
