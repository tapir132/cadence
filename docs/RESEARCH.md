...
Apple's [macOS dictation command reference](https://support.apple.com/guide/mac-help/mh40695/mac) treats spoken punctuation names and layout phrases as commands. Cadence applies `period`, `full stop`, `comma`, `colon`, `semicolon`, `question mark`, `exclamation mark`, `new line`, and `new paragraph` to each streaming hypothesis before stabilization. A partial multiword command stays provisional, and explicit punctuation replaces an adjacent model-supplied mark rather than producing `. .` or `,,`. Paragraph breaks remain atomic across a decoder pause. Layout phrases remain ordinary words when sentence grammar makes that intent clear: `a new line`, `the words new paragraph`, and `interpreting new line as` are literal, while `first thought period new line second thought` is a command. The streaming frontier uses the same distinction, so a partial literal phrase is not unnecessarily held.
...
Cadence now sends insertion units through a single serialized queue. Quick and Normal keep each committed delta intact. Essay expands the same delta into Swift `Character` values, preserving composed accents and emoji as complete graphemes. Its WPM control follows the five-character word convention, with a 40–160 WPM range. Steady, Natural, and Expressive rhythm profiles replace the old randomization switch. Natural and Expressive use centered, nonuniform jitter so most keystrokes remain near the selected pace, then apply deliberate word-, clause-, sentence-, and line-boundary pauses. The wider profile changes degree rather than inventing errors or imitating a recorded person.

The original 35 ms implementation reproduced a real Safari failure end to end: Cadence's saved transcript contained `definitely` and `The`, while the focused web editor visibly received `definitel` and `Te`. A trusted-process Safari stress experiment then delivered only 750 of 1,140 expected characters when it replaced the pasteboard and posted consecutive Command-V sequences at that cadence. The queue now treats accessible cursor advancement as an acknowledgment for complete chunks as well as individual characters, retries an unchanged paste up to twice, and stops with full-transcript recovery rather than silently skipping after repeated failure. This distinction matters when the user releases Essay's shortcut: capture finishes, but its serialized delivery task remains alive until every queued character is acknowledged. An opaque editor uses at least the conservative 120 ms timeout and may therefore run below the selected WPM. The target process and opaque focused AX window captured at session start are checked before every paste and again immediately before V-down. If focus changes, Cadence fails closed and drops the remaining queue rather than redirecting private dictation into a different window.

Every injected pasteboard item is marked [`currentHostOnly`](https://developer.apple.com/documentation/appkit/nspasteboard/contentsoptions/currenthostonly), so a paced sequence does not send each private character through Universal Clipboard. Cadence records no typing samples or biometric profile, and never adds fake mistakes or correction behavior. Non-steady rhythm variation is generated locally within fixed bounds and adds only language-aware pauses; it is a presentation control, not identity imitation.
...
Cadence compares the accessible document and cursor twice: the instant the paste queue drains, before any cleanup pass and before the person can send or edit the text, and again after completion. An exact document mutation or the exact text before the caret is definitive. Whitespace is collapsed for a secondary containment check because Terminal wraps long lines and chat composers normalize spacing while still holding every word; three real dictations on September 3, 2026 (two in Discord, one in Terminal) had been reported as lost for exactly that reason, with the Discord ones also emptied by an immediate send. A document that changed in some other way is unverifiable, not failed. When only a cursor is available, its exact advancement remains useful evidence. Quartz has no delivery receipt, so fully opaque editors remain “unverifiable” rather than being reported as successful. Only an untouched editor, a posting failure, or a focus change produces the recovery card.

## Pausing music while dictating

Apple's public [`MPRemoteCommandCenter`](https://developer.apple.com/documentation/mediaplayer/mpremotecommandcenter) is a receiver: a playback app registers handlers for commands sent by system controls. It does not expose a supported way for Cadence to send an explicit pause to whichever app owns Now Playing. A synthetic media-key event is only a play/pause toggle, so it can accidentally start media that was already paused; the private MediaRemote framework is not an acceptable release dependency.

Apple's public [`NSAppleScript`](https://developer.apple.com/documentation/foundation/nsapplescript) API can execute commands against scriptable applications through Apple events. Music and Spotify both ship scripting dictionaries with readable `player state` plus explicit `pause` and `play` commands. When the global option is enabled, Cadence checks only already-running instances, pauses each player only if it reports `playing`, records which commands succeeded, and resumes only that set when listening ends. Selecting Quick, Normal, Essay, or Custom never changes this preference. The bundle includes Apple's required [`NSAppleEventsUsageDescription`](https://developer.apple.com/documentation/bundleresources/information-property-list/nsappleeventsusagedescription), so macOS can explain and remember the one-time Automation decision.
