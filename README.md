...
The **Meeting model** selector in Notes and Settings → Notes offers **Cohere Transcribe · Beta** and **Parakeet Unified**. Cohere is the default on macOS 15 or later; Parakeet remains the default on macOS 14.2–14.x. Cohere downloads about 2.5 GB once during background setup after Cadence opens, then runs locally. Cadence loads both meeting channels and warms Cohere ahead of recording, keeping them ready while the app is open. The selector shows setup progress, readiness, and a retry option. If Cohere is still preparing, notes start with Parakeet once the live models are ready; Cohere becomes available for the next recording. Parakeet provides the live words, and Cohere re-transcribes completed phrases from their audio to revise the same lines. The choice is saved and can be changed between recordings. Cancelling a pending recording keeps model setup running. Stopping capture waits for the last revisions before saving. If Cohere fails or falls behind, the live Parakeet words remain and a notice appears. Ordinary dictation keeps its existing model and profiles.

While notes run, the microphone and a Core Audio process tap of everything the Mac plays (macOS 14.2+) feed separate local Parakeet decoders running the Accurate profile, independent of dictation. Microphone speech is labeled **You** and system audio **Them**, with separate voice-activity detection for each input. Live utterances are ordered by capture time and can be corrected in place without changing speakers. This preserves both sides during overlap and prevents louder playback from stealing the microphone’s label. Before microphone recognition, a local SpeexDSP echo canceller uses timestamped system playback as its reference to reduce speaker bleed. It adapts to the acoustic path while retaining overlapping local speech; it does not mute the microphone whenever the other person speaks. If playback capture is unavailable, microphone audio passes through. Everyone on the remote side still shares the **Them** label. Real call performance depends on the devices, room, and speaker volume; headphones also prevent acoustic bleed. The My thoughts tab is a plain editor for your own notes. When notes were started from a detected call, they stop automatically once that app releases the microphone; otherwise the stop control in the notepad ends them. Meetings are saved together in an atomic JSON file in Cadence's Application Support directory and can be copied as Markdown. Cadence never records the screen, and there is no cloud summary: the transcript and your notes are the result.

## Privacy and security

- Audio is processed locally by the downloaded Parakeet and optional Cohere Core ML models and is not saved by Cadence. Cohere briefly holds bounded phrase buffers in memory; only the text is kept.
...
CADENCE_RUN_MEETING_MODEL_TEST=1 swift test --filter MeetingTranscriptionIntegrationTests
```

See [meeting transcription verification](docs/MEETING_TRANSCRIPTION.md) for the signed-bundle capture check and live-call speaker checks.
